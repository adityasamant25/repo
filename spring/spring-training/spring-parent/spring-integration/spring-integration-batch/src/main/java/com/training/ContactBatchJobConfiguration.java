...
public class ContactBatchJobConfiguration {
	...
	@Bean
	public Step step1(StepBuilderFactory stepBuilderFactory, ItemReader<Contact> reader, ItemWriter<Contact> writer,
			ItemProcessor<Contact, Contact> processor) {
		return stepBuilderFactory.get("step1").<Contact, Contact>chunk(5).reader(reader).processor(processor)
				.writer(writer).build();
	}
	...
	public static class BatchJobFinishedListener implements ApplicationListener<JobExecutionEvent> {
		...
		public void onApplicationEvent(JobExecutionEvent event) {
			...
			this.jdbcTemplate.query("SELECT first_name, last_name, email FROM contact", (rs,
					i) -> new Contact(rs.getString("first_name"), rs.getString("last_name"), rs.getString("email")))
