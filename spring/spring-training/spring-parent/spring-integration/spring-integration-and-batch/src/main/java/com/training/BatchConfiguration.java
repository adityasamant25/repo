...
public class BatchConfiguration {
	...
	Job personEtl(JobBuilderFactory jobBuilderFactory, StepBuilderFactory stepBuilderFactory,
		...
		Step step = stepBuilderFactory.get("file-to-database").<Person, Person>chunk(5).reader(reader).writer(writer)
				.build();
	...
	CommandLineRunner runner(JobLauncher launcher, Job job, @Value("${file}") File in, JdbcTemplate jdbcTemplate) {
		return args -> {
			...
			jdbcTemplate.query("select * from PEOPLE", (resultSet, i) -> new Person(resultSet.getString("first"),
                    resultSet.getString("last"),
                    resultSet.getString("email"))).forEach(System.out::println);;
		};
	}
